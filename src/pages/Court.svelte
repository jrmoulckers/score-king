<script lang="ts">
  ...
  const nameOf = (id: string | undefined): string =>
    id ? (playerById.get(id)?.name ?? 'Someone') : '—';
...
{:else}
  ...
  <div class="row wrap seg" role="group" aria-label="Filter by time">
    {#each ranges as r (r.key)}
      <button class="chip" class:on={rangeKey === r.key} onclick={() => (rangeKey = r.key)}
        >{r.label}</button
      >
    {/each}
  </div>
  ...
  {#if gameTypes.length > 1}
    <div class="row wrap seg" role="group" aria-label="Filter by game">
      <button class="chip" class:on={filterType === 'all'} onclick={() => (filterType = 'all')}
        >All games</button
      >
  ...
  {#if result && court && hasData}
    ...
    <div class="row wrap seg" role="group" aria-label="Sort standings">
      {#each sortOptions as o (o.key)}
        <button class="chip" class:on={sortKey === o.key} onclick={() => (sortKey = o.key)}
          >{o.label}</button
        >
      {/each}
    </div>
      ...
      {#each standings as row, i (row.playerId)}
        ...
        <div class="row spread" class:me={row.playerId === meId}>
          ...
          <span class="row figures">
            <span class="fig"
              ><b class="tnum" class:lead={i === 0}>{row.wins}</b><span class="lbl">W</span></span
            >
            <span class="fig"
              ><b class="tnum">{fmtPct(row.winRate)}</b><span class="lbl">win</span></span
            >
            <span class="fig"
              ><b class="tnum">{row.avgFinish ? fmtAvg(row.avgFinish) : '—'}</b><span class="lbl"
                >avg</span
              ></span
            >
            <span class="fig">
              <b class="tnum">{row.currentStreak > 0 ? `🔥${row.currentStreak}` : '—'}</b><span
                class="lbl">run</span
              >
            </span>
          </span>
    ...
    {#if court.hottestRivalry || court.biggestMismatch}
      ...
      <div class="stack">
        {#if court.hottestRivalry}
          ...
          <div class="card stack" style="gap: 8px">
            <div class="row spread">
              <span class="row" style="gap: 8px"
                ><span aria-hidden="true">⚔️</span><strong>Hottest rivalry</strong></span
              >
        ...
        {#if court.biggestMismatch}
          ...
          <div class="card row spread tile">
            <span class="row" style="gap: 8px"
              ><span aria-hidden="true">🎯</span><strong>Biggest mismatch</strong></span
            >
            <span class="muted sm">
          ...
          <thead>
            <tr>
              ...
              {#each courtMembers as c (c.id)}
                <th scope="col" title={c.name}
                  ><Avatar name={c.name} color={c.color} size={22} /></th
                >
              {/each}
            </tr>
          </thead>
              ...
              <tr>
                ...
                {#each courtMembers as colM (colM.id)}
                  ...
                  {:else}
                    ...
                    >
                      {#if rec}<span class="tnum">{rec.wins}–{rec.losses}</span>{:else}<span
                          class="muted">·</span
                        >{/if}
                    </td>
                  {/if}
    ...
    {#if result.records.length > 0}
      ...
      <div class="card stack">
        {#each result.records as rec (rec.key)}
          {#snippet recBody()}
            <span class="row" style="gap: 8px"
              ><span aria-hidden="true">{rec.emoji}</span>{rec.label}</span
            >
            <span class="row" style="gap: 8px">
          ...
          {#if rec.gameId}
            <a
              ...
              href={`/play/${rec.gameId}`}
              use:link
              aria-label={`${rec.label} — view game`}>{@render recBody()}</a
            >
          {:else}
      ...
      <div class="card stack">
        {#if court.shame.coldestId}
          <div class="row spread">
            ...
            >
            <span class="muted sm">
              <b>{nameOf(court.shame.coldestId)}</b> ·
              <b class="tnum">{fmtPct(court.shame.coldestWinRate ?? 0)}</b>
            </span>
          </div>
        {/if}
        {#if court.shame.winlessIds.length > 0}
          <div class="row spread">
            <span class="row" style="gap: 8px"
              ><span aria-hidden="true">🫥</span>Still hunting a first win</span
            >
